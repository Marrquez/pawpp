...
Now you can rename `src/App.css` to `src/App.scss` and update `src/Menu.js` to import `src/App.scss`.
...
Import required reactstrap components within `src/Menu.js` file or your custom component files:

```js
import { Button } from 'reactstrap';
```

Now you are ready to use the imported reactstrap components within your component hierarchy defined in the render method. Here is an example [`Menu.js`](https://gist.githubusercontent.com/zx6658/d9f128cd57ca69e583ea2b5fea074238/raw/a56701c142d0c622eb6c20a457fbc01d708cb485/Menu.js) redone using reactstrap.
...
4. Add `// @flow` to any files you want to type check (for example, to `src/Menu.js`).
...
To try it, delete all the code in `src/Menu.js` and replace it with any of the examples on its website. The [Basic Example](https://reacttraining.com/react-router/web/example/basic) is a good place to get started.
...
We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `Menu.test.js` and `Menu.js` are in the same folder, the test just needs to `import App from './App'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
...
This test mounts a component and makes sure that it didn’t throw during rendering. Tests like this provide a lot of value with very little effort so they are great as a starting point, and this is the test you will find in `src/Menu.test.js`.
